					...
					<div class="container">
						<div class="inner">
							<div class="pic">
								...
								<p class="desc">
									<strong class="txt1">${ roomCount }</strong> 방 수정
								</p>
							</div>
							<div class="guide">
								<ul>
					...
					<form id="enrollForm" method ="POST">
						...
						<div class="content_body rwa">
												...
												<div class="row">
													<div class="cols">
														...
														<div class="form_group2">
															<div class="text">
																<div class="input">
																	<span><input type="text" value="${room.si} ${room.gungu} ${room.dong}"                                 
													...
													<div class="cols">
														...
														<div class="form_group2">
															<div class="col text fl">
																<div class="inner">
																	<div class="input">
																		<span> <input type="text" value="${room.main_address} - ${room.sub_address} "                       
																			disabled="disabled" />
																		</span>
																	</div>
																</div>
													...
													<div class="cols">
														...
														<div class="form_group2">
															<div class="col text fl">
																<div class="inner">
																	<div class="input">
																		<span> <input type="text" value="${room.last_address} "
																			disabled="disabled" />
																		</span>
																	</div>
																</div>
							...
							<div class="container section shadow">
								...
								<input type="hidden" name = "brokerage">
								<!--중개수수료마감-->
							</div>
							<!--금액정보마감-->
							<!--추가정보---->
						...
						<div class="row btn_wrap mt">
							<div class="button">
								...
								<div class="btn submit">
									<button id="enroll_room_and_photo_btn" type="submit" onclick ="uploadAll(); return false;">
										<span>방등록</span>
									</button>
								</div>
							</div>
...
<script>
	//이미 등록된 매물의 정보를 뷰에 보여줌 
	function setBasicRoomInfor(){
		//매물 보류  여부와 보류내용 설정
		if('${room.close}' == 0)//매물이면
			$("input[name=close_chk]:eq(0)").trigger('click');
		else//매물보류이면
			$("input[name=close_chk]:eq(1)").trigger('click');
		var close_reason = ('${room.close_reason}').split(':');
		for(var i = 0; i < close_reason.length; i++){
			if(close_reason[i]  == '계약완료'){
				//매물보류이유 : 계약완료 선택상태로
				$("#sale_hold_select").find("option:eq(1)").prop("selected", true);
				setCloseReason(1);
				continue;
			}
			if(close_reason[i]  == '임대인요청'){
				//매물보류이유 : 임대인요청 선택상태로
				$("#sale_hold_select").find("option:eq(2)").prop("selected", true);
				setCloseReason(2);
				continue;
			}
			if(close_reason[i]  == '기타'){
				//매물보류이유 : 기타 선택상태로
				$("#sale_hold_select").find("option:eq(3)").prop("selected", true);
				setCloseReason(3);
				continue;
			}
			if(0 < close_reason[i].length){
				//매물보류이유 : 기타사유값 설정
				$("#close_reason_content").val(close_reason[i]);
				continue;
			}
		}
		//건물형태 체크 설정
		if('${room.apartment}' == 1)
			$("input[name=building_type_chk]:eq(0)").trigger('click');
		else if('${room.officetel}' == 1)
			$("input[name=building_type_chk]:eq(1)").trigger('click');
		else if('${room.house}' == 1)
			$("input[name=building_type_chk]:eq(2)").trigger('click');
		else if('${room.normal_room}' == 1)
			$("input[name=building_type_chk]:eq(3)").trigger('click');
		else if('${room.shop}' == 1)
			$("input[name=building_type_chk]:eq(4)").trigger('click');
		else if('${room.office}' == 1)
			$("input[name=building_type_chk]:eq(5)").trigger('click');
		else if('${room.building}' == 1)
			$("input[name=building_type_chk]:eq(6)").trigger('click');
		else if('${room.redevelopment}' == 1)
			$("input[name=building_type_chk]:eq(7)").trigger('click');
		else if('${room.event}' == 1)
			$("input[name=building_type_chk]:eq(8)").trigger('click');
	    ...
	    .trigger('change');
 		if('${room.rooftop}' == 1){
 		    ...
 		    .val('옥탑방')
 		    .trigger('change');
 		} else if('${room.semi_basement}' == 1){
 			$('#floor_no')
 		    .val('반지하')
 		    .trigger('change');
 		} else if('${room.floor_no}' == -1){
 			$('#floor_no')
 		    .val('지하')
 		    .trigger('change');
 		}
 		...
 		//중공년도 설정 
		$("input[name='built']").val('${room.built}'); 
 		//방개수 설정 
 		$("input[name='room_count']" ).val('${room.room_count}');	
 		//욕실수 설정
 		$("input[name='bathroom_count']" ).val('${room.bathroom_count}');
 		
 		//방형태 설정 -  오픈형:0/분리형:1/복층형:2
		if('${room.room_type}' == 0)
			$("input[name=room_type_chk]:eq(0)").trigger('click');
		else if('${room.room_type}' == 1)
			$("input[name=room_type_chk]:eq(1)").trigger('click');
		else if('${room.room_type}' == 2)
			$("input[name=room_type_chk]:eq(2)").trigger('click');
		
 		//공급면적설정
		$("input[name='leased_area']" ).val('${room.leased_area}');
		$("input[name=leased_area]").trigger('onchange');
		//전용면적 설정
		$("input[name='exclusive_area']").val('${room.exclusive_area}');
		$("input[name=exclusive_area]").trigger('onchange');
		 
		 
	/* re
		$('input[name=sale_hold_select]')
	    .val('two')
	    .trigger('change');
 */
...
<script>
	...
	function uploadTempPhoto(target_photo_cnt){
		...
		$.ajax({
	        ...
	        success: function (data) {
	        	...
	        	else if(0 < data.tempPath.length){
	...
	function setAddressAtr(){
		...
		formData["new_address"] = "${postAddress.roadAddress}";
...
<script>
	...
	function check(){
		...
		if(all_floor=='전체층선택'){//전체층선택 입력확인
			alert('전체층을 선택해주세요.');
			return false; 
		}
		if(floor_no=='해당층선택'){//해당층선택 입력확인
			alert('해당층을 선택해주세요.');
			return false; 
		}
		if(built =='' ){//준공년도 입력확인 
			...
			/* alert('준공년도를 입력해주세요.');
			return false; */
		}else if(built !='' && built.length != 4){//준공년도 입력확인   //4자리 확인
			alert('준공년도를 정확히 입력해주세요.');
			return false;
		}
		
		if(room_count ==''){//방개수 입력확인
			room_count = 0;
			/* alert('방개수를 입력해주세요.');
			return false; */
		...
		if(standard_deposit ==''){//기준가 입력확인
			...
			return false;
		}else if (12 < standard_deposit.length) {//기준가 입력확인 //기준가 범위를 초과하면
			alert('기준가 범위를 초과하였습니다.');
			return false;
		}

		if(sale_price ==''){//매매가 입력확인
			sale_price = 0;
			/* alert('매매가를 입력해주세요.');
			return false; */
			...
			return false;
		}else if (12 < sale_price.length) {//매매가 입력확인 //매매가 범위를 초과하면
			alert('매매가 범위를 초과하였습니다.');
			return false;
		}
	    
		if(charter_deposit ==''){//전세 입력확인
			charter_deposit = 0;
			/* alert('전세가를 입력해주세요.');
			return false; */
			...
			return false;
		}else if (12 < charter_deposit.length) {//전세 입력확인 //전세 범위를 초과하면
		...
		if(deposit ==''){//보증금 입력확인
			...
			return false;
		}else if (10 < deposit.length) {//보증금 입력확인 //전세 범위를 초과하면
			alert('보증금 범위를 초과하였습니다.');
			return false;
		}

		if(monthly_rent ==''){//월세 입력확인
			monthly_rent = 0;
			/* alert('월세를 입력해주세요.');
			return false; */
	    ...
	    if(brokerage ==''){//중개수수료 입력확인
			 brokerage = 0;
			/*  alert('중개 수수료를 입력해주세요.');
			return false;  */
		}else if (brokerage !='' && !(/^[0-9]+$/).test(brokerage)) {//관리비 입력확인 //정수 형태가 아니면
			alert('중개 수수료를 정확히 입력해주세요.');
			return false;
		}else if (brokerage !=''&& 8 < brokerage.length) {//관리비 입력확인 //전세 범위를 초과하면
			alert('중개 수수료 범위를 초과하였습니다.');
			return false;
		} 
	    
		if (3000 < other_option.length){//기타옵션  // 글자수제한
			other_option
			alert('기타옵션 3000자 초과');
			return false;
		}
		if (50 < parking_option.length){//주차옵션  // 글자수제한
			alert('주차옵션 50자 초과');
			return false;
		}
	    /* 
		...
		if (20 < move_day.length){//입주가능일  // 글자수제한
			alert('입주가능일 20자 초과');
			return false;
		}/* 
		...
		if (3000 < detail.length){//상세설명 // 글자수제한
			alert('상세설명3000자 초과');
			return false;
		}/*
		...
		if (3000 < requirement.length){//요구사항  // 글자수제한
			alert('요구사항 3000자 초과');
			return false;
		}/*
		...
		if (50 < movie_url.length){//동영상경로  // 글자수제한
			alert('동영상 경로를 정확히 입력해주세요.');
			return false;
		}

		if (contact_who_1 =='' || contact_phone_1 =='' ){//임대인 연락처 입력확인
			/* alert('임대인 연락처를 입력해주세요.');
			return false; */
		}else if (20 < contact_who_1.length){//임대인 연락처   // 글자수제한
			alert('임대인 연락를 정확히 입력해주세요.');
			return false; 
		}else if ( contact_who_2 =='' || contact_phone_2 =='' ){//관리인 연락처 입력확인
			/* alert('관리인 연락처를 입력해주세요.');
			return false; */
		}else if (20 < contact_who_2.length){//관리인 연락처   // 글자수제한
			alert('관리인 연락처를 정확히 입력해주세요.');
			return false; 
		}else if ( contact_who_3 =='' || contact_phone_3 =='' ){//관리부동산 연락처 입력확인
			/* alert('관리부동산 연락처를 입력해주세요.');
			return false; */
		}else if (20 < contact_who_3.length){//관리부동산 연락처   // 글자수제한
			alert('관리부동산 연락처를 정확히 입력해주세요.');
			return false; 
		}
		return true;
	}	
	...
	$(function() {
		 ...
		 $(document).on("change","#sale_hold_select",function(){
			...
			setCloseReason(select_index);//매물보류 이유 선택한 select의 option 위치로 설정
		});
	});
	//매물보류 이유 설정
	function setCloseReason(select_index){
		if(select_index == 0){
			close_reason = ':::';
		} else if(select_index == 1){
			close_reason = '계약완료:::';
